...
import { Search, DollarSign, TrendingUp, TrendingDown, Wallet, X, User, Users } from "lucide-react";
...
export default function TallyPage() {
  ...
  const financialSummary = useMemo(() => {
    ...
    dataToUse.forEach((lead) => {
      ...
      const calculatedPending = TOTAL_AMOUNT - (regAmount + concessionAmount + partAmount);
  ...
  const accountsSummary = useMemo(() => {
    ...
    leadsWithFinancialData.forEach(lead => {
      if (lead.currentOwnerId && summary[lead.currentOwnerId]) {
        ...
        const pending = 7000 - (regAmount + concAmount + partAmount);
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-foreground" data-testid="total-amount">
                  ₹{financialSummary.totalRegistration.toFixed(2)}
                </div>
                      ...
                      filteredLeads.map((lead) => (
                        <TableRow key={lead.id} data-testid={`row-lead-${lead.id}`}>
                          ...
                          <TableCell className="text-right font-medium text-destructive">
                            {`₹${(7000 - ((lead.registrationAmount ? parseFloat(lead.registrationAmount) : 0) + (lead.concession ? parseFloat(lead.concession) : 0) + (lead.partialAmount ? parseFloat(lead.partialAmount) : 0))).toFixed(2)}`}
                          </TableCell>
