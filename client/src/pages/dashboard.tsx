...
export default function Dashboard() {
  ...
  const activeCategoryFilter = useMemo(() => {
    const userRole = (user as any)?.role;
    // Managers, Admin Organizers, and Team Leads can filter by category or see all
    if (userRole === 'manager' || hasManagerPermissions(userRole) || userRole === 'team_lead') {
      return managerCategoryFilter;
    }
    return selectedCategory; // HR/Accounts/Session Organizer use their localStorage category
  }, [(user as any)?.role, managerCategoryFilter, selectedCategory]);
