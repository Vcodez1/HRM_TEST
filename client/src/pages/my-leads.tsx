...
export default function MyLeadsPage() {
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      const newCategory =
        localStorage.getItem("selectedCategory") || "Client Hiring";
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["/api/my/leads", (user?.role === 'accounts' || user?.role === 'session_organizer' || isSessOrg) ? 'all' : selectedCategory, user?.role, currentAdminSubRole],
    queryFn: async () => {
  ...
  const { data: accountsPendingData, isLoading: accountsPendingLoading } = useQuery({
    queryKey: ["/api/my/accounts-pending", selectedCategory],
    queryFn: async () => {
      const params = new URLSearchParams();
      params.append("category", selectedCategory);
      const url = `/api/my/accounts-pending?${params}`;
      const response = await fetch(url, { credentials: "include" });
                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          >
                            ...
                            {/* Show category badge for Accounts users */}
                            {user?.role === 'accounts' && lead.category && (
                              <Badge
                                variant="secondary"
                                className="text-xs"
                                data-testid={`category-${lead.id}`}
                              >
                                {lead.category}
                              </Badge>
                            )}
                            ...
                            <FormItem>
                              ...
                              <Select onValueChange={field.onChange} defaultValue={field.value}>
                                ...
                                <SelectContent>
                                  ...
                                  <SelectItem value="JAVA-FS">JAVA Full Stack</SelectItem>
                                  <SelectItem value="PYTHON-FS">PYTHON Full Stack</SelectItem>
                                  <SelectItem value="MERN-FS">MERN Full Stack</SelectItem>
                                  <SelectItem value="MEAN-FS">MEAN Full Stack</SelectItem>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        >
                          ...
                          {/* Show category badge for Accounts and Session Organizer users */}
                          {(user?.role === 'accounts' || user?.role === 'session_organizer' || isSessOrg) && lead.category && (
                            <Badge
                              variant="secondary"
                              className="text-xs"
                              data-testid={`category-${lead.id}`}
